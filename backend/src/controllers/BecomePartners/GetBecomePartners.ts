...
export default class GetBecomePartners extends Controller {
        ...
        try {
            ...
            if (search) {
                ...
                filter.$or = [
                    ...
                    { typeOfPartnership: searchRegex },
                    { author: searchRegex }
                ];
            }

            // Récupération des partenaires avec pagination et tri
            ...
            // Compte total des partenaires (pour la pagination)
            const totalPartners = await db.collection("partners").countDocuments(filter);
